...
- **Rich profile fields + equipment consolidation + numeric axis values (Phase 5+):** Three related to-do items captured 2026-05-14 — discuss and implement together in a future phase:
  1. **Rich bar-owner-profile fields in onboarding and UI** — `bar-owner-profile.json` contains rich fields the web UI does not yet surface: `background.drinking_frequency`, `background.household_context`, `background.vocabulary_preference`, `archetypes` (array of `{name, description}` objects — e.g. "The Minimalist", "The Experimenter"), and `archetype_notes`. Add questions for these in the onboarding wizard and expose them in the Profile tab. Do NOT add these fields to `barkeeper.json` or `inventory.json`.
  2. **Equipment consolidation** — Equipment currently lives in `bar-owner-profile.json` AND `barkeeper.json`. The canonical source of truth should be `inventory.json` only (the `equipment` object already exists there per D-11). Remove the equipment field from both `bar-owner-profile.json` and `barkeeper.json`. Update onboarding to write equipment to `inventory.json`. Update `normalize.js` to strip equipment from profile and barkeeper on load.
  3. **Numeric axis values in profile storage and UI** — The Profile tab flavor axes currently store string labels ("Lean A", "Strong A", "Strong B", "Lean B", "Middle") in `bar-owner-profile.json`. Replace these with numeric `0.0–1.0` float values that map directly to the slider position (0.0 = Strong A, 0.5 = Middle, 1.0 = Strong B). Remove the display of "Lean A" / "Strong A" / etc. heading labels from the Profile tab UI entirely — show only the labeled slider bars with numeric values. Update `normalize.js` to migrate legacy string labels to their numeric equivalents on load.
