...
- **BUG-03 (open, to fix in Phase 4):** After performing an import, saving inventory fails with "Save failed: data/inventory.json does not match [sha]". This is a GitHub API 409 Conflict — the SHA cached in the app's in-memory state no longer matches the file's actual SHA on GitHub. Root cause: the import flow writes to `inventory.json` on GitHub but does not update `State._shas['inventory']` to the new SHA returned by the write response, leaving the cached SHA stale. On the next `State.save('inventory')` call the old SHA is sent, GitHub rejects it. Fix: after any import write that touches `inventory.json`, update `State._shas['inventory']` (and any other affected keys) to the SHA returned in the GitHub API PUT response before returning. Pattern to follow: inspect how `State.save()` already handles the returned SHA and replicate that in the import path. Planner must also check whether the same stale-SHA bug can occur after image uploads or any other path that writes directly via `GitHubAPI.writeJSON()` outside of `State.save()`.
