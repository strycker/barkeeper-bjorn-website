...
**Status:** SHIPPED + VERIFIED 2026-05-27 — commits `946e3c9` (1/3) + `9527dce` (2/3) + `cc5df53` (3/3) + four post-ship fixes (`3129e81` reclassify v1, `5646147` tolerant lookup + autosave + is_original, `4044db5` version bump v2_2, `d2adef0` manual pool migration, `2754c2b` CLASSICS_DB lexical lookup + shim resolve + recommender pool writes, `506854d` WeakMap handler swap + optimistic UI + 3-retry backoff). User UAT confirmed working: chips render uniformly across all tabs, classics show correct seed_id-backed metadata, Recommender ♥/☆/✓ toggles update instantly with optimistic UI, per-tab remove handlers fire the right toast and stay on the right tab, save reliability holds under rapid actions. 62/62 deterministic tests green throughout. Legacy `State.get('recipes')` compat shim retained as a transitional bridge for non-recipes-view callers (recommender.js reads, export.js, claude-api.js context, profile.js count) — queued cleanup commit, does NOT block Phase 7 UAT resumption.
